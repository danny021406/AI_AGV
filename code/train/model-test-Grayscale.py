...
# load in the data
import pandas as pd
track_log = pd.read_csv('./deep_store.csv',names=['left_wheel_dir','left_wheel_speed', 'right_wheel_dir', 'right_wheel_speed', 'filename'], skiprows=1)
...
train_samples, validation_samples = train_test_split(track_log, test_size=0.2, random_state=42)
# print(train_samples)
...
left_dir_output = Dense(1, name='left_dir_output')(x)
right_speed_output = Dense(1, name='right_speed_output')(x)
right_dir_output = Dense(1, name='right_dir_output')(x)
...
def save_model(model_name):
    ...
    model_name = ('./model_dir_0_1/' + model_name  + '_' + timestamp + '.h5')
