...
import cv2
import numpy as np
dir_log = pd.read_csv('../../data/csv/deep_store.csv',usecols=[1,3])
speed_log = pd.read_csv('../../data/csv/deep_store.csv',usecols=[2,4])
filename_list= pd.read_csv('../../data/csv/deep_store.csv',usecols=[5])
...
class Queue():  
    ...
    def show(self): 
        ...
        for i in range(10):
            ...
            queue_tmp.append(self.queue[self.front + i + 1])
        return queue_tmp
    def q_size(self):
...
def generator(samples, batch_size=64):
    ...
    while 1: 
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            right_dirs = []
            target = []
            pre_image=load_in_img('../../data/train_data/test - 9/black.png')
#             print(pre)
...
#                     queue_image.enqueue(aug_image)
#                     if(queue_image.q_size() == 10):
# #                         print(queue_image.q_size())
                    images.append(pre_image)
                    images.append(aug_image)
                    pre_image = aug_image.copy()
...
#             y_train = np.hstack((left_speeds,left_dirs,right_speeds,right_dirs))
            ...
            X_train = X_train.reshape(-1, 2, 200, 200, 1)
...
from keras.layers import Lambda, Conv2D, MaxPooling2D, Dropout, Dense, Flatten, Input, Activation, LSTM, TimeDistributed, ConvLSTM2D, GRU, Bidirectional
...
INPUT_SHAPE = (2 , 200, 200, 1) # height width
DROP_PROB = 0.7
...
cnn = Sequential()
# cnn.add(ConvLSTM2D(12, kernel_size=5, padding = "same", batch_input_shape=(1, None, 200, 200, 1), return_sequences=False, stateful=True))
cnn.add(Conv2D(24,(3,3), activation='relu', strides=(2, 2), input_shape=(200, 200, 1)))
cnn.add(Conv2D(24,(5,5), activation='relu', strides=(2, 2)))
cnn.add(Conv2D(36,(5,5), activation='relu', strides=(2, 2)))
cnn.add(Conv2D(48,(5,5), activation='relu', strides=(2, 2)))
cnn.add(Conv2D(64,(3,3), activation='relu'))
cnn.add(Conv2D(64,(3,3), activation='relu'))
cnn.add(Dropout(DROP_PROB))
cnn.add(Flatten())
# rnn = Sequential()
# rnn.add(LSTM(2, input_shape=(2, 200)))
# rnn.add(LSTM(2, input_dim=int(28), input_length=int(2)))
# dense = Sequential()
# dense.add(Dense(50))
# dense.add(Dense(10))
...
# x = dense(model) # add dense
# x = TimeDistributed(Conv2D(24, 3, 3, activation='relu', subsample=(2, 2)))(main_input)
# x = TimeDistributed(Conv2D(24, 5, 5, activation='relu', subsample=(2, 2)))(x)
# x = TimeDistributed(Conv2D(36, 5, 5, activation='relu', subsample=(2, 2)))(x)
# # x = TimeDistributed(Conv2D(48, 5, 5, activation='relu', subsample=(2, 2)))(x)
# # x = ConvLSTM2D(36, 5, 5, activation='relu', subsample=(2, 2))(x)
# x = TimeDistributed(Conv2D(64, 3, 3, activation='relu'))(x)
# x = TimeDistributed(Conv2D(64, 3, 3, activation='relu'))(x)
# x = TimeDistributed(Dropout(DROP_PROB))(x)
# # x = TimeDistributed(Flatten())(x)
# # x = ConvLSTM2D(24, 3, 3, activation='relu', subsample=(2, 2))(main_input)
# # x = ConvLSTM2D(24, 5, 5, activation='relu', subsample=(2, 2))(x)
# # x = ConvLSTM2D(36, 5, 5, activation='relu', subsample=(2, 2))(x)
# # x = ConvLSTM2D(48, 5, 5, activation='relu', subsample=(2, 2))(x)
...
# x = ConvLSTM2D(36, 5, 5, activation='relu', subsample=(2, 2))(x)
# x = Flatten()(x)
x = LSTM(100, return_sequences = True)(model) 
x = LSTM(50)(x)
# x = Flatten()(model)
# x = Dense(100)(x)
x = Dense(50)(x)
...
left_dir_output_ = Dense(5, name='left_dir_output_')(x)
...
right_dir_output_ = Dense(5,name='right_dir_output_')(x)
...
def save_model(model_name):
    ...
    model_name = ('../../data/model/model_dir_ver5/' + model_name  + '_' + timestamp + '.h5')
